    ...
    for(var i = 0; i < queryOrder; i++) {
        ...
        for(var key in queryOrder[i]) {
            if(queryOrder[i].hasOwnProperty(key)) {
                var sendMethodParent = queryOrder[i][key] === 'locally' ? this.broadcastObj : this.broadcastObj.childProcClient;
                var sendMethod = (queryOrder[i][key] === 'locally' ? this.broadcastObj.sendCommand : this.broadcastObj.childProcClient.sendCommand).bind(sendMethodParent, key, command);
